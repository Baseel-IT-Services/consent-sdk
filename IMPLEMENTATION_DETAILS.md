...
23. [Publishing & Release Process](#section-23-publishing--release-process)
...
| npm publish configuration | ✅ Complete | All 4 packages published (`@baseel/types@0.1.0`, `@baseel/loader@0.0.1`, `@baseel/consent-web-component@0.1.0`, `@baseel/consent-react@0.1.0`) — see §23. Remaining gap: no CI/CD automation, still a manual process. |
...
| npm publish | ✅ Done — all 4 packages published, see §23 |
...
| "How does the SDK get deployed to merchants?" | Published to the public npm registry under the `@baseel` scope (`@baseel/types@0.1.0`, `@baseel/loader@0.0.1`, `@baseel/consent-web-component@0.1.0`, `@baseel/consent-react@0.1.0`). A merchant simply runs `npm install @baseel/consent-react` (or `@baseel/consent-web-component`) — see §23 for the release process. |
...
## Section 23: Publishing & Release Process

This section documents how the SDK actually went from source code to a real, publicly installable product — added after the initial publish, since the rest of this document (dated 2026-07-03) predates it. It covers both the one-time setup and the repeatable release process a maintainer follows for any future version.

### 23.1 Where the code lives

GitHub: [`https://github.com/Baseel-IT-Services/consent-sdk`](https://github.com/Baseel-IT-Services/consent-sdk) (org `Baseel-IT-Services`), default branch `main`.

### 23.2 Where the packages live

All four packages are published on the public npm registry under the `@baseel` scope, owned by the npm organization `baseel`:
...
Only the **root** monorepo `package.json` is `private: true` (see §7) — the 4 individual packages are public, real npm packages, each with `"publishConfig": {"access": "public"}`.

**Why `@baseel` and not `@baseel-sdk`:** the packages were originally coded with an `@baseel-sdk` scope. An npm scope can only be published to if it matches your username or an npm organization you belong to — and the team's actual npm organization is named `baseel`, not `baseel-sdk`. All four `package.json` `name` fields (and every internal cross-package `dependencies` reference) were renamed from `@baseel-sdk/*` to `@baseel/*` to match before publishing.

### 23.3 One-time account setup

1. **Org membership.** Publishing under `@baseel` requires at least the `developer` role in the npm org `baseel`. Verify with `npm org ls baseel`. Current members: `pareshdeshmukh` (owner), `karanjaiswal0000`, `rani_kumari_baseel`, `vigneshkumar.d` (all `developer`).
2. **CLI login.** `npm login` in a real interactive terminal (opens a browser tab). Being logged into npmjs.com in a browser does **not** authenticate the CLI — they are separate sessions/tokens.
3. **Two-factor authentication.** npm now requires 2FA (or a granular access token with publish + 2FA-bypass permission) before `npm publish` succeeds. Without it: `403 E403: Two-factor authentication or granular access token with bypass 2fa enabled is required to publish packages.` Enabled at npmjs.com → Account Settings → Configure 2FA → an authenticator app → **"Authorization and Publishing"** mode (not "Authorization only", which doesn't cover publish).

### 23.4 Per-release process

```mermaid
flowchart TD
    A["npm install && npm run build (root)"] --> B["Optional: npm publish --access public --dry-run per package"]
    B --> C["cd packages/types && npm publish --access public"]
    C --> D["cd ../loader && npm publish --access public"]
    D --> E["cd ../consent-web-component && npm publish --access public"]
    E --> F["cd ../consent-react && npm publish --access public"]
    F --> G["Verify: npm view @baseel/<name> version"]
```

Publish order is not arbitrary — it follows the same dependency graph as the build (`types` → `loader` + `consent-web-component` → `consent-react`), because each package's `package.json` pins an exact version of the sibling package it depends on. Publishing out of order would publish a package referencing a dependency version that doesn't exist on the registry yet.

Each `npm publish` call may prompt for a 2FA one-time code inline, or print a browser URL (`https://www.npmjs.com/auth/cli/...`) to approve. **This step is interactive and must be run by a human in a real terminal** — it cannot be scripted or run non-interactively (e.g. by an AI agent or an unattended CI job) against a personal account's browser-based 2FA. For a future automated/CI pipeline, the documented alternative is an npm **Granular Access Token** scoped to the `@baseel` packages with write permission and 2FA-bypass-for-publish explicitly enabled, stored as a CI secret — not what was used for this initial publish.

**Verification:** `npm view @baseel/<name> version`, or `GET https://registry.npmjs.org/@baseel/<name>/<version>` (200 = live). Note a harmless propagation quirk: the *unversioned* packument endpoint can return a stale `404` for a few minutes right after a fresh publish, due to npm's Fastly CDN edge-caching a prior negative lookup. The version-pinned endpoint and npm's search index (`https://registry.npmjs.org/-/v1/search?text=%40baseel`) reflect the publish immediately.

### 23.5 Publishing a future version

Bump the `version` field in the relevant package's `package.json` (and any dependent package's pinned reference to it), rebuild, then repeat the dry-run/publish/verify steps for just that package.

### 23.6 What was fixed before the first publish

Two verified issues that were corrected before publishing (npm package metadata for a given version is effectively immutable once published, so these had to be right beforehand):

- All four packages' `repository.url` and `homepage` fields pointed at a placeholder URL (`github.com/baseel-sdk/baseel-sdk`) instead of the real GitHub repo — corrected to `https://github.com/Baseel-IT-Services/consent-sdk`.
- The `@baseel-sdk` → `@baseel` scope rename described in §23.2.

---

*Generated by Claude Code — based on complete codebase analysis of branch `1-pashe-1-configuration` as of 2026-07-03. Section 23 added subsequently, following the SDK's actual publish to GitHub and the npm registry.*
